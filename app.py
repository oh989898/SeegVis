...
@app.route('/geteeg', methods=['GET'])
def geteeg():
    if request.method == "GET":
        try:
            c1 = int(request.args.get('c1'))
            c2 = int(request.args.get('c2'))
            raw = mne.io.read_raw_edf("./mydata/zhangjie-10sz.edf")
            sfreq=raw.info['sfreq']
            data,times=raw[c1-1:c1,int(sfreq*1):int(sfreq*700)]
            plt.plot(times,data.T)
            data,times=raw[c2-1:c2,int(sfreq*1):int(sfreq*700)]
            plt.plot(times,data.T)
            plt.title("Channels")
            plt.savefig('./static/001.png')
        except Exception as e:
            return jsonify({'result': False, 'msg': "系统出错！\n" + repr(e)})
        return jsonify({'result': True, 's1_s2': c1+c2})
    return jsonify({'result': False, 'msg': "Not GET Request!"})
